export function UpdateGameTimer(state) {
    ...
    let timer1 = row1.getElementsByTagName('td')[row1.getElementsByTagName('td').length - 1];
    let minutes1 = Math.floor(state.time1left / (60 * 1000));
    let seconds1 = Math.floor(state.time1left / 1000) % 60;
    timer1.textContent = minutes1.toString().padStart(2, '0') + ':' +
        seconds1.toString().padStart(2, '0');
    let timer2 = row2.getElementsByTagName('td')[row2.getElementsByTagName('td').length - 1];
