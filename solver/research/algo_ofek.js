...
var analElem = getElemId("anal"),
  numTrialsElem = getElemId("num-trials");
...
function newGame() {
  ...
  for (var i = 0; i < board.length; i++) {
    board[i] = new Array(9);
    for (var a = 0; a < board[i].length; a++) board[i][a] = 0;
  }
...
function drawGrid() {
    ...
    if (bestChild !== null) {
      ...
      for (i = 0; i < 9; i++)
        for (a = 0; a < 9; a++) {
          ...
          if (squareRoot !== null) {
            brush.fillStyle = getWeightedStyle(
              bestTries,
              squareRoot.totalTries,
              xTurnGlobal
            );
            brush.fillRect(
              i * squareWidth,
  ...
  if (!drawWeights && prevMove && !over) {
    ...
    if (board[nextCenter[0]][nextCenter[1]] < 3 && xTurnGlobal) {
      ...
      brush.fillRect(
        (nextCenter[0] - 1) * squareWidth,
        (nextCenter[1] - 1) * squareWidth,
        3 * squareWidth,
        3 * squareWidth
      );
...
function getWeightedStyle(bestTries, tries, xTurn) {
  if (xTurn) return "rgba(102, 162, 255, " + tries / bestTries + ")";
...
function drawPiece(x, y) {
  ...
  switch (color) {
    ...
    case "O":
      ...
      brush.fillText(
        color + "",
        Math.floor(x / 3) * squareWidth * 3 + squareWidth * 1.5,
        Math.floor(y / 3 + 1) * squareWidth * 3 - o4 * 1.5
      );
      break;
    default:
      return;
...
function drawBoard() {
  ...
  for (var I = 1; I < 9; I += 3)
    for (var A = 1; A < 9; A += 3)
      if (board[I][A] === 5 || board[I][A] === 6) drawPiece(I, A);
      else
        for (var i = I - 1; i <= I + 1; i++)
          for (var a = A - 1; a <= A + 1; a++)
            if (board[i][a] !== 0) drawPiece(i, a);
}
...
function getMove(xloc, yloc) {
  ...
  if (xloc < left || xloc > left + boardWidth || yloc > boardWidth)
    return [-1, -1];
  return [
    Math.floor((xloc - left) / squareWidth),
    Math.floor(yloc / squareWidth),
  ];
}
...
function legalMove(tboard, move, prevMove, output) {
  ...
  if (tboard[move[0] - (move[0] % 3) + 1][move[1] - (move[1] % 3) + 1] > 2) {
    if (output) alert("Square already finished");
    return false;
  }
  if (prevMove) {
    if (
      tboard[(prevMove[0] % 3) * 3 + 1][(prevMove[1] % 3) * 3 + 1] < 3 &&
      (prevMove[0] % 3 !== Math.floor(move[0] / 3) ||
        prevMove[1] % 3 !== Math.floor(move[1] / 3))
    ) {
      if (output) alert("Wrong square!");
      return false;
    }
  }
  return true;
...
function setTurn(turn, move) {
  ...
  if (gameOver(board, color, move)) over = color;
  else if (totalEmptyGlobal === 0) over = "tie";

  xTurnGlobal = turn;
  prevMove = move;

  globalRoot = MCTSGetNextRoot(move);
  if (globalRoot) globalRoot.parent = null;
  ...
  if (over) {
    setTimeout(function () {
      ...
      if (tie)
        if (anti) str += " (anti + tie tic tac toe)";
        else str += " (tie tic tac toe)";
      else if (anti) str += " (anti tic tac toe)";
      alert(str);
    }, 100);
    stopPonder();
  }
...
boardui.addEventListener("mousedown", function (e) {
  if (e.which === 3) return;
  if (
    (aiTurn !== "null" && xTurnGlobal === (aiTurn === "first")) ||
    aiTurn === "both"
  )
    return;
  if (over) {
  ...
  if (!legalMove(board, move, prevMove, true)) return;
...
function playMove(tboard, move, xturn) {
  var color = xturn ? 1 : 2;
  var centerx = move[0] - (move[0] % 3) + 1,
    centery = move[1] - (move[1] % 3) + 1;
  var startx = move[0] - (move[0] % 3),
    starty = move[1] - (move[1] % 3);
  tboard[move[0]][move[1]] = color;
  if (localWin(tboard, color, move, startx, starty))
    tboard[centerx][centery] = color + 4;
  else if (squareFull(tboard, startx, starty)) tboard[centerx][centery] += 2;
  else return false;
  return true;
}
...
function playMoveGlobal(tboard, move, xturn) {
  var emptyLeft =
    emptySpotsGlobal[(move[0] - (move[0] % 3)) / 3][
      (move[1] - (move[1] % 3)) / 3
    ];
...
function gameOver(tboard, color, m) {
  ...
  if ((move[0] - (move[0] % 3)) / 3 !== 2 - (move[1] - (move[1] % 3)) / 3)
    ...
    for (i = 1, a = 7; i < 9; i += 3, a -= 3)
      if (tboard[i][a] !== color) return false;
  return true;
}

function tieGame(tboard, m) {
  for (var i = 1; i < 9; i += 3)
    for (var a = 1; a < 9; a += 3) if (tboard[i][a] < 3) return false;
  return true;
}
...
function updateAnalysis() {
  ...
  analElem.innerHTML =
    ...
    range[2] +
    " Certainty-" +
    (globalRoot && globalRoot.totalTries > 0
      ? (resultCertainty(globalRoot) * 100).toFixed(0)
      : "0") +
    "%";
...
function startPonder() {
  pondering = setInterval(function () {
    if (!globalRoot) globalRoot = createMCTSRoot();
    var startTime = new Date().getTime();
    var tempCount = 0;
    while (new Date().getTime() - startTime < 30 && !stopChoose) {
      globalRoot.chooseChild(
        simpleBoardCopy(board),
        simpleSpotsCopy(emptySpotsGlobal),
        totalEmptyGlobal
      );
      tempCount++;
    }
    if (
      numChoose3 &&
      (tempCount < numChoose3 / 10 ||
        tempCount < numChoose2 / 10 ||
        tempCount < numChoose1 / 10)
    )
      stopChoose = true;
    else {
    ...
    if (numPonders % 10 === 0 && drawWeights)
      if (hoveredMove) drawHover(hoveredMove);
      else drawBoard();
    updateAnalysis();
  }, 1);
}
...
function getMCTSDepthRange() {
  var root,
    range = new Array(3);
  for (
    range[0] = -1, root = globalRoot;
    root && root.children;
    range[0]++, root = leastTriedChild(root)
  );
  for (
    range[1] = -1, root = globalRoot;
    root && root.children;
    range[1]++, root = mostTriedChild(root)
  );
  root = globalRoot;
  if (root.totalTries > (root.hits + root.misses) * 3) range[2] = "Tie";
  else if (root.hits > root.misses === xTurnGlobal) range[2] = "X";
  else if (root.hits < root.misses === xTurnGlobal) range[2] = "O";
  else range[2] = "Tie";
  return range;
}

function MCTSGetChildren(father, tboard) {
  if (father.result !== 10) return [];
  ...
  if (father.lastMove) {
    var nextCenter = [
      (father.lastMove[0] % 3) * 3 + 1,
      (father.lastMove[1] % 3) * 3 + 1,
    ];
...
function getEmptySpots(tboard) {
  ...
  var count, I, A, i, a;
  for (i = 0; i < emptySpots.length; i++) emptySpots[i] = new Array(3);
  ...
  for (I = 1; I < 9; I += 3)
    for (A = 1; A < 9; A += 3) {
      ...
      if (tboard[I][A] < 3)
        for (i = I - 1; i <= I + 1; i++)
          for (a = A - 1; a <= A + 1; a++) if (tboard[i][a] === 0) count++;
...
function MCTSSimulate(father, tboard, emptySpots, totalEmpty, playMoveResult) {
  if (father.result !== 10) return father.result;

  if (
    playMoveResult === 1 &&
    totalEmpty <= 54 &&
    gameOver(tboard, father.turn ? 6 : 5, father.lastMove)
  )
    if (tie) return (father.result = father.turn !== anti ? -1 : 0);
    else return (father.result = anti ? 1 : -1);

  if (totalEmpty === 0)
    return (father.result = tie ? (father.turn !== anti ? 1 : -1) : 0);

  var lm = father.lastMove,
    turn = father.turn,
    done = false;
  var nextCenter;
  ...
  while (!done) {
    ...
    currentEmpty =
      emptySpots[(nextCenter[0] - (nextCenter[0] % 3)) / 3][
        (nextCenter[1] - (nextCenter[1] % 3)) / 3
      ];
    if (currentEmpty !== 0) {
      count = Math.floor(Math.random() * currentEmpty);
      outer: for (x = nextCenter[0] - 1; x <= nextCenter[0] + 1; x++)
  ...
  if (tie) return father.turn !== anti ? (turn ? 0 : -1) : turn ? 0 : 1;
  if ((turn === father.turn) !== anti) return -1;
  return 1;
}
...
function runMCTS(time) {
  ...
  while ((new Date().getTime() - startTime) / 1e3 < time) {
    ...
    if (
      globalRoot.children.length < 2 ||
      error < certaintyThreshold ||
      certainty > 0.9
    )
      return;
  }
...
function getCertainty(root) {
  ...
  var ratioWins =
    bestChild.hits < bestChild.misses
      ? (bestChild.hits / bestChild.misses) * 2
      : (bestChild.misses / bestChild.hits) * 3;
...
function mostTriedChild(root, exclude) {
  ...
  for (var i = 0; i < root.children.length; i++)
    if (
      root.children[i] !== exclude &&
      root.children[i].totalTries > mostTrials
    ) {
...
function bestRatioChild(root) {
  ...
  var child = root.children[0],
    bestRatio = child.misses / child.hits;
  if (root.children.length === 1) return child;
...
function leastTriedChild(root) {
  var leastTrials = root.totalTries + 1,
    child = null;
  if (!root.children) return null;
  for (var i = 0; i < root.children.length; i++)
...
function MCTSGetNextRoot(move) {
  if (!globalRoot || !globalRoot.children) return null;
  for (var i = 0; i < globalRoot.children.length; i++)
    if (
      globalRoot.children[i].lastMove[0] === move[0] &&
      globalRoot.children[i].lastMove[1] === move[1]
    ) {
...
class MCTSNode {
    ...
    if (this.result !== 10)
      ...
      if (unexplored > 0) {
        ...
        for (i = 0; i < this.children.length; i++)
          if (this.children[i].totalTries === 0) {
            ...
            if (unexplored === 0) {
              ...
              emptyLeft =
                emptySpots[(lastMove[0] - (lastMove[0] % 3)) / 3][
                  (lastMove[1] - (lastMove[1] % 3)) / 3
                ];
              playMoveResult = playMoveEmptyLeft(
                board,
                lastMove,
                !this.children[i].turn,
                emptyLeft
              );
        ...
        var bestChild = this.children[0],
          bestPotential = MCTSChildPotential(this.children[0], this.totalTries),
          potential;
  ...
  backPropogate(simulation) {
    if (simulation === 1) this.hits++;
    else if (simulation === -1) this.misses++;
    this.totalTries++;
    if (this.parent !== null) this.parent.backPropogate(-simulation);
  }
}
...
function speedTest(numSimulations) {
  ...
  console.log(
    numberWithCommas(Math.round(numSimulations / elapsedTime)) +
...
function simpleBoardCopy(board) {
  ...
  for (var i = 0; i < 9; i++) {
    ...
    for (var a = 0; a < 9; a++) simpleCopy[i][a] = board[i][a];
  }
  return simpleCopy;
}
...
function simpleSpotsCopy(spots) {
  ...
  for (var i = 0; i < 3; i++) {
    ...
    for (var a = 0; a < 3; a++) simpleCopy[i][a] = spots[i][a];
  }
  return simpleCopy;
}
...
function parseOver(over) {
  switch (over) {
    case "tie":
      if (tie) return anti ? 2 : 1;
      return 0;
    case 5:
      if (anti) return tie ? 1 : 2;
      return tie ? 0 : 1;
    case 6:
      if (anti) return tie ? 0 : 1;
      return 2;
  }
}
...
function testStats(timeToThink, numTrials) {
  ...
  for (var I = 0; I < numTrials; I++) {
    ...
    while (!over) {
      var startTime = new Date().getTime();
      if (!root) root = createMCTSRoot();
      while ((new Date().getTime() - startTime) / 1e3 < timeToThink) {
        for (var i = 0; i < 100; i++) root.chooseChild(simpleBoardCopy(board));
        var error = getCertainty(root);
        if (root.children.length < 2 || error < certaintyThreshold) break;
      }
      var bestChild = mostTriedChild(root, null);
      ...
      var color = xTurnGlobal ? 5 : 6;
      if (gameOver(board, color, bestMove)) over = color;
      else if (tieGame(board)) over = "tie";
      ...
      if (root.children) {
        for (var i = 0; i < root.children.length; i++)
          if (
            root.children[i].lastMove[0] === bestMove[0] &&
            root.children[i].lastMove[1] === bestMove[1]
          ) {
...
function evaluateOver(lastMove) {
  var color = xTurnGlobal ? 5 : 6;
  if (gameOver(board, color, lastMove)) over = color;
  else if (tieGame(board)) over = "tie";
}
...
function testMultithreading(numTrials, timeToThink, init, v1, v2) {
  ...
  if (numTrials === 0) {
    console.log(
      v1 > v2 ? "Multi-threading is better!" : "Multi-threading is worse :/"
    );
    return;
...
function testExpansionConstants(c1, c2, numTrials, timeToThink, output) {
  ...
  for (var I = 0; I < numTrials; I++) {
    ...
    while (!over) {
      ...
      if (!r) r = createMCTSRoot();
      while ((new Date().getTime() - startTime) / 1e3 < timeToThink) {
        for (var i = 0; i < 100; i++) r.chooseChild(simpleBoardCopy(board));
        var error = getCertainty(r);
        if (r.children.length < 2 || error < certaintyThreshold) break;
      }
      ...
      if (r1.children) {
        for (var i = 0; i < r1.children.length; i++)
          if (
            r1.children[i].lastMove[0] === bestMove[0] &&
            r1.children[i].lastMove[1] === bestMove[1]
          ) {
            r1 = r1.children[i];
            break;
          }
      ...
      if (r2.children) {
        for (var i = 0; i < r2.children.length; i++)
          if (
            r2.children[i].lastMove[0] === bestMove[0] &&
            r2.children[i].lastMove[1] === bestMove[1]
          ) {
            r2 = r2.children[i];
            break;
          }
    ...
    switch (parseOver(over)) {
      case 0:
        if (output) console.log("tie");
        break;
      case 1:
        if (I % 2 === 0) {
          v1++;
          if (output) console.log("c1 wins");
        } else {
          v2++;
          if (output) console.log("c2 wins");
        }
        break;
      case 2:
        if (I % 2 === 0) {
...
function findBestExpansionConstant(
  seed,
  timeToThink,
  bound,
  numSimulations,
  ...
  var round1 = testExpansionConstants(
    seed,
    prollyGreater ? seed + bound : seed - bound,
    numSimulations,
    timeToThink,
    false
  );
  if (round1[1] > round1[0])
    findBestExpansionConstant(
      prollyGreater ? seed + bound : seed - bound,
      timeToThink,
      bound / 2,
      numSimulations,
    ...
    var round2 = testExpansionConstants(
      seed,
      prollyGreater ? seed - bound : seed + bound,
      numSimulations,
      timeToThink,
